...
class Marble_Creator(mp.Process):
    ...
    def __init__(self, settings, mario):
...
class Bagger(mp.Process):
    ...
    def __init__(self, settings, luigi, mario):
        mp.Process.__init__(self)
        # TODO Add any arguments and variables here
        self.num_to_bag = settings[NUMBER_OF_MARBLES_IN_A_BAG]
    ...
    def run(self):
        ...
        bag = Bag()
        while True:
            if bag.get_size() < self.num_to_bag:
                marble = self.luigi.recv()
                if marble == "no more marbles":
                    break
                bag.add(marble)
            else:
                self.mario.send(bag)
...
class Wrapper(mp.Process):
    ...
    def __init__(self, settings, luigi, counter):
        mp.Process.__init__(self)
        # TODO Add any arguments and variables here
    ...
    def run(self):
        ...
        with open(BOXES_FILENAME, "w") as f:
            while True:
                gift = self.luigi.recv()
                if gift == "no more gifts":
                    break
...
def main():
    ...
    mario1, luigi1 = mp.Pipe()
    mario2, luigi2 = mp.Pipe()
    mario3, luigi3 = mp.Pipe()
    ...
    processes = []
    processes.append(Marble_Creator(settings, mario1))
    processes.append(Bagger(settings, luigi1, mario2))
    processes.append(Assembler(settings, luigi2, mario3))
    processes.append(Wrapper(settings, luigi3, gift_counter))
    ...
    log.write(f"Gifts created: {gift_counter.value}")
