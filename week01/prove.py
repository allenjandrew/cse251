...
def draw_squares(tur, gand=None):
    """Draw a group of squares"""
    # x1,x2,y1,y2 = get_cords(quad)
    for x in range(-300, 350, 200):
        for y in range(-300, 350, 200):
            if gand: gand.acquire()
            draw_square(tur, x - 50, y + 50, 100)
            if gand: gand.release()


def draw_circles(tur, gand=None):
...
def draw_triangles(tur, gand=None):
...
def draw_rectangles(tur, gand=None):
...
def run_with_threads(tur, log, main_turtle):
    ...
    threads.append(threading.Thread(target=draw_squares, args=(tur,gand)))
    threads.append(threading.Thread(target=draw_circles, args=(tur,gand)))
    threads.append(threading.Thread(target=draw_triangles, args=(tur,gand)))
    threads.append(threading.Thread(target=draw_rectangles, args=(tur,gand)))
