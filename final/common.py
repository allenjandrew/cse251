...
class Tree:
    ...
    def add_person(self, person):
        if self.does_person_exist(person.get_id()):
            print(
                f"ERROR: Person with ID = {person.get_id()} Already exists in the tree"
            )
        else:
    ...
    def add_family(self, family):
        if self.does_family_exist(family.get_id()):
            print(
                f"ERROR: Family with ID = {family.get_id()} Already exists in the tree"
            )
        else:
    ...
    def display(self, log):
        ...
        for family_id in self.__families:
            ...
            if husband == None:
                ...
                if parent_fam_id in self.__families:
                    ...
                    log.write(
                        f"  Husband Parents: {father.get_name()} and {mother.get_name()}"
                    )
                else:
                    log.write(f"  Husband Parents: None")
            ...
            if wife == None:
                ...
                if parent_fam_id in self.__families:
                    ...
                    mother = self.get_person(parent_fam.get_wife())
                    log.write(
                        f"  Wife Parents: {father.get_name()} and {mother.get_name()}"
                    )
                else:
                    log.write(f"  Wife Parents: None")
        ...
        log.write(
            f"Max generations                     : {self._count_generations(self.__start_family_id)}"
        )
        log.write(
            f"People connected to starting family : {self._test_number_connected_to_start()}"
        )
