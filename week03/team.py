        ...
        for row in range(self.size):
            for col in range(self.size):
                if self.highlighting[row][col]:
                    print(f'{bcolors.OKBLUE}{bcolors.BOLD}{self.board[row][col]}{bcolors.ENDC} ', end='')
                else:
...
def main():
    ...
    with mp.Pool(8) as p:
        hilites = p.map(board.find_word, words)
        for foo in hilites:
            if len(hilites) != len(words):
                print(len(hilites))
                print(len(words))
